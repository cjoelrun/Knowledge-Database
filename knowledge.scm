...
(define stringify_subject (lambda (symbols string)
                            (if (not (null? symbols))
                                (if (not (null? (cdr symbols))) 
                                    (stringify_subject (cdr symbols) (string-append string (string-append (symbol->string (car symbols)) " ")))
                                    (stringify_subject (cdr symbols) (string-append string (symbol->string (car symbols)))))
                                string)))

(define stringify_description (lambda (symbols string)
                                (if (not (null? symbols))
                                    (if (not (null? (cdr symbols))) 
                                        (stringify_description (cdr symbols) (string-append string (string-append (symbol->string (car symbols)) " ")))
                                        (stringify_description (cdr symbols)(string-append string (symbol->string (car symbols)))))
                                    string)))
